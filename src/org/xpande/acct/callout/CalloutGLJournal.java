...
public class CalloutGLJournal extends CalloutEngine
{
	...
	public String period (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		// Xpande. Gabriel Vila. 21/12/2018.
		// Tomo organización del contexto.
		...
		{
			// Xpande. Gabriel Vila. 21/12/2018.
			// El período debe primero buscarse en calendario de la organización y luego en el calendario de la compañia
			// Comento codigo original y sustituyo
			String sql = " SELECT C_Period_ID "
					+ " FROM C_Period "
					+ " WHERE C_Year_ID IN "
					+ "	(SELECT C_Year_ID FROM C_Year WHERE C_Calendar_ID ="
					+ "  (SELECT C_Calendar_ID FROM AD_OrgInfo WHERE AD_Org_ID=" + AD_Org_ID + "))"
					+ " AND ? BETWEEN StartDate AND EndDate"
					+ " AND IsActive='Y'"
					+ " AND PeriodType='S'";

			C_Period_ID = DB.getSQLValueEx(null, sql);

			if (C_Period_ID > 0){
				mTab.setValue("C_Period_ID", new Integer(C_Period_ID));
			}
			else{
				// Tomo período desde calendario asociado a la compañia
				sql = " SELECT C_Period_ID "
						+ " FROM C_Period "
						+ " WHERE C_Year_ID IN "
						+ "	(SELECT C_Year_ID FROM C_Year WHERE C_Calendar_ID ="
						+ "  (SELECT C_Calendar_ID FROM AD_ClientInfo WHERE AD_Client_ID=" + AD_Client_ID + "))"
						+ " AND ? BETWEEN StartDate AND EndDate"
						+ " AND IsActive='Y'"
						+ " AND PeriodType='S'";
